...
import { isSome, matchOption, matchResult, matchResultAsync, none, some, type Option } from "./fp.js";
...
import type { PhotoAsset, ScanCounts, ScanResult, ScanReviewBusyAction } from "./types.js";

export interface ScanController {
  applyTimestamp(): Promise<void>;
  loadScanResult(): Promise<void>;
  prefillScanUntil(): void;
...
function scanReviewBusyCopy(ctx: AppContext): { title: string; detail: string } {
...
function busyActionIs(ctx: AppContext, expected: ScanReviewBusyAction): boolean {
  return matchOption(ctx.state.scanReviewBusyAction, {
...
function updateScanReviewBusyOverlay(ctx: AppContext): void {
...
function setScanReviewBusy(ctx: AppContext, action: Option<ScanReviewBusyAction>): void {
...
function visibleScanReviewAssets(ctx: AppContext): PhotoAsset[] {
...
function finishedStats(counts: ScanCounts): string {
  return scanStat("Review", counts.review || 0, "nonzero-good") +
    scanStat("Low conf.", counts.low_confidence || 0, "nonzero-warn") +
    scanStat("Other", counts.unknown || 0, "") +
...
function visibleSelectableReviewThumbs(): HTMLElement[] {
  return [...document.querySelectorAll<HTMLElement>("#photoGrid .photo-thumb")].filter(thumb =>
    !thumb.classList.contains("is-ref") &&
    !thumb.classList.contains("is-neg") &&
    !thumb.classList.contains("is-tagged")
  );
}

function allVisibleScanReviewSelected(ctx: AppContext): boolean {
...
function renderScanReview(ctx: AppContext): void {
...
function renderScanReviewFilterBtns(ctx: AppContext): void {
  const container = el("scanFilterBtns");
  const counts = new Map<string, number>();
  ctx.state.scanReviewAssets.forEach(asset => {
    if (asset.pet_name) counts.set(asset.pet_name, (counts.get(asset.pet_name) || 0) + 1);
  });
  matchOption(ctx.state.scanReviewFilter, {
    some: name => { if (!counts.has(name)) ctx.state.scanReviewFilter = none; },
    none: () => undefined,
  });
  ...
  [...container.querySelectorAll<HTMLElement>("[data-filter-pet]")].forEach(btn => {
    btn.onclick = () => {
...
function toggleVisibleScanReviewSelection(ctx: AppContext): void {
  if (isScanReviewBusy(ctx.state)) return;
  const thumbs = visibleSelectableReviewThumbs();
...
async function scanAssignSelected(ctx: AppContext): Promise<void> {
  if (isScanReviewBusy(ctx.state) || !ctx.state.selectedCrops.size) return;
...
async function scanAddSelectedAsRefs(ctx: AppContext): Promise<void> {
  if (isScanReviewBusy(ctx.state) || !ctx.state.selectedCrops.size) return;
  const petNames = [...ctx.state.scanReviewSelectedPets];
  ...
  await matchResultAsync(result, {
    ok: async data => {
      ...
      if (isActivePet) await requireAction(ctx, "loadRefs")(petName);
      await requireAction(ctx, "refreshState")();
...
function showScanResult(ctx: AppContext, result: ScanResult): void {
...
export function createScanController(ctx: AppContext): ScanController {
  const renderScanReviewPetBtns = (): void => {
    ...
    [...container.querySelectorAll<HTMLElement>(".scan-pet-choice")].forEach(btn => {
      btn.onclick = () => {
        const pet = btn.dataset.pet;
        if (!pet) return;
        if (ctx.state.scanReviewSelectedPets.has(pet)) ctx.state.scanReviewSelectedPets.delete(pet);
        else ctx.state.scanReviewSelectedPets.add(pet);
  ...
  const updateScanReviewSelectAllBtn = (): void => {
  ...
  const applyTimestamp = async (): Promise<void> => {
  ...
  const prefillScanUntil = (): void => {
    const scanUntil = input("scanUntil");
    if (!scanUntil.value) scanUntil.value = new Date().toISOString().slice(0, 10);
  };

  window.addEventListener("beforeunload", event => {
    if (!ctx.state.scanRunning) return;
