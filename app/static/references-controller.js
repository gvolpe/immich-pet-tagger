...
import { err, isSome, matchOption, matchOptionAsync, matchResult, matchResultAsync, none, ok, some } from "./fp.js";
import { cropSelectionKey, markGridItems, renderPhotoItems } from "./photo-grid.js";
import { negativeKeys, requireAction, resetModes, resetSelection, } from "./app-context.js";
let addByIdMode = "ref";
function clearBorderlineTimer(ctx) {
...
function renderRefs(ctx, assets) {
...
function mergeCropRefs(existing, newCrops) {
    const seen = new Set();
...
function newCropFromAsset(assetId, asset) {
...
function openAddByIdModal(mode) {
    addByIdMode = mode;
...
async function validateAndAddNegative(ctx, assetId) {
...
async function addSingleAssetRef(ctx, petName, assetId, asset) {
...
export function createReferencesController(ctx) {
    ...
    const addCropsToPetRefs = async (petName, newCrops, loadedRefs = none) => {
        const existingRefs = await matchOptionAsync(loadedRefs, {
            some: refs => ok(refsToCrops(refs)),
            none: async () => {
                const result = await ctx.services.refs.list(petName);
                return matchResult(result, {
                    ok: data => ok(refsToCrops(data.assets)),
    ...
    const removeRef = async (assetId, cropIdx = null) => {
    ...
    const assignSelected = async () => {
        if (!ctx.state.selectedCrops.size)
            return;
        await matchOptionAsync(ctx.state.activePet, {
            none: () => Promise.resolve(),
            some: async (pet) => {
    ...
    const openAddById = () => {
        if (isSome(ctx.state.activePet))
            openAddByIdModal("ref");
    };
    const openAddNegById = () => openAddByIdModal("neg");
