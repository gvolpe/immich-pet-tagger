...
import { err, fromNullable, matchOption, matchResult, ok, tryCatchSync } from "./fp.js";
function parseOptionalBbox(value) {
    return matchOption(fromNullable(value), {
        none: () => ok(null),
        some: json => matchResult(tryCatchSync(() => JSON.parse(json), error => `Invalid crop bbox: ${error instanceof Error ? error.message : String(error)}`), {
            ok: bbox => ok(bbox),
            err: message => err(message),
        }),
    });
}
export function readCropData(thumb) {
    ...
    return matchOption(assetId, {
        none: () => err("Photo thumb is missing its asset id"),
        some: id => matchResult(bbox, {
            ok: value => ok({ asset_id: id, crop_idx: cropIdx, bbox: value }),
            err: message => err(message),
        }),
    });
}
export function getCropData(thumb) {
    return matchResult(readCropData(thumb), {
        ok: crop => crop,
        err: message => { throw new Error(message); },
    });
}
export function cropSelectionKey(crop) {
    return crop.crop_idx != null ? `${crop.asset_id}_${crop.crop_idx}` : crop.asset_id;
}
export function scanReviewAssetKey(asset) {
    return asset.crop_idx != null ? `${asset.id}_${asset.crop_idx}` : asset.id;
}
export function renderPhotoItems(asset, threshold, immichUrl) {
    const scoreBadge = asset.score != null
        ...
        : "";
    const details = [];
    if (asset.score_margin != null)
        details.push(`margin ${Math.round(asset.score_margin * 100)}%`);
    if (asset.unknown_score != null)
        details.push(`unknown ${Math.round(asset.unknown_score * 100)}%`);
    if (asset.detection_conf != null)
        details.push(`YOLO ${Math.round(asset.detection_conf * 100)}%`);
    ...
    if (asset.crops && asset.crops.length > 0) {
        return asset.crops.map(crop => makeItem(`${asset.id}_${crop.crop_idx}`, `/api/crop/${asset.id}?bbox=${crop.bbox.join(",")}`, crop.crop_idx, crop.bbox));
    }
...
export function markGridItems(assets, refsItems, negativeKeys) {
    ...
    assets.forEach(asset => {
        ...
        keys.forEach(key => {
            ...
            if (!thumb)
                return;
            if (refKeys.has(key) || refKeys.has(asset.id))
                thumb.classList.add("is-ref");
            if (negSet.has(key) || negSet.has(asset.id))
