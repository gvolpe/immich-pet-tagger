...
import { matchOption, matchResult, matchResultAsync, none, some } from "./fp.js";
...
export function createNegativesController(ctx) {
    const updateNegStatus = () => {
    ...
    const addSelectedAsNegatives = async () => {
        if (!ctx.state.selectedCrops.size)
            return;
        const selected = [...ctx.state.selectedCrops.values()];
        const keys = new Set(selected.map(cropSelectionKey));
        const assetIds = [...new Set(selected.map(crop => crop.asset_id))];
        const result = ctx.state.scanReviewMode
