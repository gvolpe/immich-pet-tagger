...
import { err, fromNullable, matchOption, matchOptionAsync, matchResult, matchResultAsync, none, ok, some } from "./fp.js";
import { activePetName, clearActivePet, requireAction, } from "./app-context.js";
let petToEdit = none;
let petToDelete = none;
let allImportPeople = [];
let importSelectedPerson = none;
function optionalDate(value) {
    return value || null;
}
function validateDateRange(sinceRaw, untilRaw) {
    const dateRe = /^\d{4}-\d{2}-\d{2}$/;
    if (sinceRaw && !dateRe.test(sinceRaw))
        return err('Invalid "since" date');
    if (untilRaw && !dateRe.test(untilRaw))
        return err('Invalid "until" date');
    if (sinceRaw && untilRaw && sinceRaw > untilRaw)
        return err('"Since" must be before "until"');
    return ok({ since: optionalDate(sinceRaw), until: optionalDate(untilRaw) });
}
function validatePetPayload(prefix) {
    ...
    return matchResult(validateDateRange(input(prefix === "pet" ? "petSince" : "editPetSince").value, input(prefix === "pet" ? "petUntil" : "editPetUntil").value), {
        ok: range => ok({ name, description, since: range.since, until: range.until }),
        err: message => err(message),
    });
}
function validateImportPayload(person) {
    const description = input("importPetDescription").value.trim();
    if (!description)
        return err("Description is required");
    return matchResult(validateDateRange(input("importPetSince").value, input("importPetUntil").value), {
        ok: range => ok({
            person_id: person.id,
            name: person.name,
            description,
...
function renderImportPeople(ctx, people) {
...
export function createPetDialogsController(ctx) {
    const openAddPet = () => {
        input("petName").value = "";
    ...
    const confirmDeletePet = async (localOnly) => {
    ...
    const handlePersonCardClick = (card) => {
    ...
    const backToImportPicker = () => {
        el("importDetailModal").classList.remove("open");
        el("importPickerModal").classList.add("open");
    };
    const submitImportPet = async () => {
                        ...
                        await matchResultAsync(result, {
                            ok: async (data) => {
                                ...
                                toast(data.ref_count > 0
                                    ...
                                    : `Imported ${data.name} with 0 refs. No animals were detected in the reference photos. Add refs manually.`, data.ref_count > 0 ? "success" : "warn");
                            },
